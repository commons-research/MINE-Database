...
def main():
    ...
    df = lotus[["structure_inchikey", "structure_smiles_2D"]].drop_duplicates().reset_index(drop=True)
    ...
    df = df[df["structure_smiles_2D"].str.contains("C", case=False)].reset_index(drop=True) # Keep only rows with 'C' or 'c' in the smiles
