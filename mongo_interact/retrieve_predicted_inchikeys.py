...
def retrieve_predicted_inchikeys(
    connection_string, database_name, taxonomy_rank, taxonomy_value
):
    # Connect to MongoDB
    ...
    print(
        f"Found {len(starting_compounds_list)} starting compounds with {taxonomy_rank} = {taxonomy_value}"
    )
    ...
    for starting_compound in starting_compounds_list:
        ...
        reactions = reactions_col.find(
            {"Reactants": {"$elemMatch": {"$elemMatch": {"$eq": reactant_id}}}}
        )

        # Debug: Count and list all reactions retrieved for the starting compound
        reactions_list = list(reactions)
        print(
            f"Found {len(reactions_list)} reactions for starting compound {starting_inchikey}"
        )

        for reaction in reactions_list:
            for product in reaction["Products"]:
                product_id = product[1]
                product_compound = compounds_col.find_one(
                    {"_id": product_id, "Type": "Predicted"}
                )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Retrieve predicted InChIKeys for starting compounds based on organism taxonomy rank."
    )
    parser.add_argument(
        "--connection_string", required=True, help="MongoDB connection string"
    )
    parser.add_argument(
        "--database_name", required=True, help="Name of the MongoDB database"
    )
    parser.add_argument(
        "--taxonomy_rank",
        default="organism_taxonomy_05order",
        help="The taxonomy rank field to filter on (default: organism_taxonomy_05order)",
    )
    parser.add_argument(
        "--taxonomy_value",
        required=True,
        help="The value of the taxonomy rank to filter on",
    )

    args = parser.parse_args()

    retrieve_predicted_inchikeys(
