...
def main(limit=None):
    ...
    grouped_reactions = defaultdict(lambda: {'starting_compound': None, 'product_compounds': []})

    # Fetch total reactions count
    total_reactions = reactions_col.count_documents({})  # Count all documents in the reactions collection
    ...
    for reaction in tqdm(reactions, total=total_reactions, desc="Processing reactions"):
        ...
        if starting_compound:
            ...
            predicted_products = extract_predicted_products(reaction['Products'], reaction['Operators'])
    ...
    for inchikey, reaction_data in grouped_reactions.items():
        unique_products = {p['inchikey']: p for p in reaction_data['product_compounds']}.values()  # Remove duplicates by InChIKey
    ...
    with open('./data/output/npc_expanded_grouped_starting_and_predicted_products.json', 'w') as f:
    ...
    with open(csv_file, 'w', newline='') as f:
        ...
        # Write header
        writer.writerow(['Starting Compound SMILES', 'Starting Compound InChIKey',
                         'Product Compound SMILES', 'Product Compound InChIKey'])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--limit', type=int, help='Limit the number of reactions to process.')
