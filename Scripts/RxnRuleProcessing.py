        ...
        for filename in names:
            if ".dat" in filename:
                ...
                try:
                    ...
                    op_data["Products"][filename[:-4]] = ";".join(
                        products.strip().split()
                    )
...
operators = pandas.read_csv(
    "../minedatabase/data/EnzymaticReactionRules.tsv", delimiter="\t"
).set_index("Name")
error_ops = set(
    re.findall(
        "Warning: Unbalanced Reaction produced by (\d.\d+.-?\d+.\w)",
        open("/Applications/PyCharm.app/Contents/bin/error.txt").read(),
    )
)
print("%s operators will be rotated" % len(error_ops))
print(error_ops)
operators.update(
    operators.filter(items=error_ops, axis="index")["Products"].apply(rotate_products)
)
