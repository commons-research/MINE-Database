    ...
    for i, row in compounds.iterrows():
        if row["SMILES"]:
            ...
            if mol:
                ...
                if pic_dir:
                    rc = subprocess.call(
                        "/Applications/ChemAxon/JChem/bin/molconvert -o %s/temp.png png:-a,w500 -s "
                        "'%s'" % (pic_dir, row["SMILES"].strip()),
                        shell=True,
                    )
                    if not rc:
    ...
    for i, row in reactions.iterrows():
        if row["Equation (Abbreviations)"]:
            ...
            rxn["Reactants"], rxn["Products"] = utils.parse_text_rxn(
                row["Equation (Abbreviations)"], " = ", " + ", abrv
            )
            rxn["InChI_hash"] = utils._calculate_rxn_hash(
                mine_db, rxn["Reactants"], rxn["Products"]
            )
